...
def involved(courses, person):
    result = {}
    for time in courses:
        for c in courses[time]:
            classes = []
            for k in courses[time][c]:
                if person == courses[time][c][k]:
                    classes.append(c)
        result.setdefault(time, classes)
    return result
...
def test():
    ...
    assert involved(courses, 'Dave') == {'apr2012': [
        'cs101', 'cs387'], 'feb2012': ['cs101']}

    assert involved(courses, 'Peter C.') == {
        'apr2012': ['cs262'], 'feb2012': ['cs101']}

    assert involved(courses, 'Dorina') == {'jan2044': ['cs001']}

    assert involved(courses, 'Peter') == {}

    assert involved(courses, 'Robotic') == {}

    assert involved(courses, '') == {}
