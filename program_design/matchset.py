...
def matchset(pattern, text):
    ...
    lit: the pure literals will be matdched.
    seq: use a seqence of pattern to match one by one.
    alt: return union set of two pattern matches text sets
    dot: matches any character.
    oneof: matches any of the characters in the string it is
     called with. oneof('abc') will match a or b or c.
    '''
...
def test():
    ...
    assert matchset(('oneof', 'abc'), 'aabc123') == set(['abc123'])
